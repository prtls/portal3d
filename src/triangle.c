...
void draw_texel(
   ...
   // Draw the actual pixel, passing for color value the width of the texture
buffer * y (to get y-coord of
   // the texture buffer) offset by x (to get x coord). This is the same little
formula we use for the color buffer
   // to fetch a 2D coordinate from a 1d array.
   int texIndex = ((texture_width * texture_y) + texture_x) % (texture_width *
texture_height); draw_pixel(x, y, texture[texIndex]);
...
void draw_textured_triangle(
        int x0, int y0, float z0, float w0, float u0, float v0, // including z
and w because every pixel needs to carry depth int x1, int y1, float z1, float
w1, float u1, float v1, // information to prevent affine texture mapping
disortion int x2, int y2, float z2, float w2, float u2, float v2, uint32_t*
texture ) {
// loop all pixels of triangle to render them based on color fetched from
corresponding uv coordinate in texture array
...
c_uv);
    ...
    // Only render if delta y != 0 (prevents bug where odd edges are rendered)
    if (y2 - y1 != 0) {
        // From height (y) of middle (where we just left off) to height (y) of
bottom tip of triangle... for (int y = y1; y <= y2; y++) {
            // Calculate the beginning (left-boundary) of this scanline
            int x_start = x1 + (y - y1) * inv_slope_1;
            // Calculate the end (right-boundary) of this scanline
            int x_end = x0 + (y - y0) * inv_slope_2;
